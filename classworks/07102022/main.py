...
##2. Прибавить 3
##Программа для исполнителя Калькулятор – это последовательность команд. Сколько существует программ, для которых при
##исходном числе 2 результатом является число 20, и при этом траектория вычислений содержит число 10 и не содержит число15?
##'''
##def f(x, n):
##    if x > n or x == 15: 
##        return 0
...
##которым присвоены номера:
##1. Вычесть 1
##2. Убрать последнюю цифру справаПервая команда уменьшает число на 1. Вторая команда убирает последнюю справа цифру, например, для числа 110
##результатом работы данной команды будет являться число 11.
##''' 
##l = [0] * 100
##l[int('110111', 2)] = 1
