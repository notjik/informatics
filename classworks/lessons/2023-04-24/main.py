...
"""
### TODO: Ответ — 22
##with open('data/24-247.txt') as f:
##    data = f.readlines()
##res = []
##for s in data:
##    lt = set(s)
...
##        i += 1
##    for symb in lt:
##        res.append((symb, i, s.count(symb)))
##print(sorted(filter(lambda a: a[1] == max(res, key=lambda b: b[1])[1], res), key=lambda c: c[2])[0][2])


"""
25 (А. Богданов) Назовём маской числа последовательность цифр, в которой также могут встречаться
следующие символы:
...
Найдите 5 наименьших натуральных чисел, которые кратны 73 и соответствуют маске 12345*76. Выведите эти
числа в порядке возрастания, справа от каждого числа выведите результат деления числа на 73.
"""
### TODO: Ответ —
res = []
insert = ['']
for ins in insert:
    n = int('12345{}76'.format(ins))
    if not n % 73:
        res.append((n, n // 73))
i = 1
while len(res) < 5:
    insert = [''.join(i) for i in product(map(str, range(10)), repeat=i)]
    for ins in insert:
        n = int('12345{}76'.format(ins))
        if not n % 73:
            res.append((n, n // 73))
    i += 1
print(*sorted(res, key=lambda x: x[0])[:5], sep='\n')


"""
26 (А. Кабанов) В текстовом файле записан набор натуральных чисел. Гарантируется, что все числа различны.
Для каждой пары различных чисел из набора вычисляется значение K – количество чисел из набора, меньших
среднего арифметического этой пары. Необходимо определить количество пар чисел с ненулевым значением
K, кратным ста, а также наибольшее K среди этих пар.
Входные данные представлены в файле 26-47.txt следующим образом. Первая строка содержит целое число N
– общее количество чисел в наборе. Каждая из следующих N строк содержит одно число, не превышающее 109.
...
7
59 В
примере рассмотрим пары со значением K кратным трём. В данном случае есть 14 подходящих пар.
Например, для пары (9, 17) значение K равно 6 (6 чисел из набора меньше среднего арифметического 13). В
ответе надо записать числа 14 и 6.
"""
### FIXME: Ответ —
##with open('data/26-47t.txt') as f:
##    data = list(map(int, f.readlines()))
##res = []
##all_K = []
##for i in combinations(data, r=2):
##    tmp = data[:]
##    tmp.remove(i[0]).remove(i[1])
##    K = len([j for j in data.remove(i[0]).remove(i[0]) if j < (sum(i) / 2)])
##    print(i, K)
##    if not K % 6:
##        all_K.append(K)
##print(len(all_K), max(all_K))
