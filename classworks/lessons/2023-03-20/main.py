...
'''
### TODO: Ответ – 720
##vowel = [''.join(i) for i in product('ОЕА', repeat=2)]
...
##    if any(data[i+j] < avg for j in range(2)) and any(data[i+j] % 100 == 13 for j in range(2)):
##        c += 1
...
def f(x, n, mul):
    ...
    if x == n:
        return 1
    return f(x+1, n, True) if mul else f(x+1, n, False) + f(x*2, n, True) + f(x*3, n, True)
