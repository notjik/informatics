...
Игроки ходят по очереди, первый ход делает Петя. За один ход игрок может уменьшить количество камней в три раза 
(количество камней, полученное при делении, округляется до меньшего) или убрать из кучи 12 камней. 
Например, из кучи из 35 камней можно получить кучу из 11 или 23 камней. Игра завершается в тот момент, 
когда количество камней в куче становится не более 12. Победителем считается игрок, сделавший последний ход. 
В начальный момент в куче было S камней (S ≥ 13).
...
################################################################################
answ20 = [s for s in range(13, 1000) if game(s) == 2]
print(min(answ20), max(answ20))
print(len([s for s in range(13, 1000) if game(s) == -2]))
