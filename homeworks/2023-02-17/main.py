...
'''
21. Исполнитель А12S преобразует целое число, записанное на экране. У исполнителя три команды,
...
10 и т. д.). Программа для исполнителя А12S – это последовательность команд.
...
#     if x == n:
#         return 1
#     return A12S(x + 1, n) + A12S(x + 2, n) + A12S(x + x - 1, n)
#
#
# print(A12S(3, 10))
...
'''
22. Текстовый файл 24-s1.txt состоит не более чем из 106
 символов и содержит только заглавные
буквы латинского алфавита (ABC…Z). Текст разбит на строки различной длины. Необходимо найти
строку, содержащую наименьшее ненулевое количество пар соседних букв, которые стоят в таком же
порядке и в алфавите (например, AB, BC, CD и т.д.). Если таких строк несколько, надо взять ту,
которая в файле встретилась раньше. Определите, какая буква встречается в этой строке чаще всего.
Если таких букв несколько, надо взять ту, которая стоит последней в алфавите. Запишите в ответе эту
букву, а затем – сколько раз она встречается во всем файле.
Пример. Исходный файл:
ZZQABA
ZALMAC
KRAKUT
В этом примере в первой и второй строках по одной подходящей паре (AB и LM), в третьей таких пар
нет. Берём первую строку, т.к. она находится в файле раньше. В этой строке чаще других встречаются
буквы Z и A (по два раза), выбираем букву Z, т. к. она позже стоит в алфавите. В ответе для этого
примера надо записать Z3, так как во всех строках файла буква Z встречается 3 раза.
'''
# # TODO: Ответ – W38473
# with open('data/24-s1.txt') as f:
#     lines = f.readlines()
# checkers = [chr(i)+chr(i+1) for i in range(ord('A'), ord('Z'))]
# mn = 10**10
# res_s = ''
# for s in lines:
#     summ_pairs = sum(s.count(i) for i in checkers)
#     if summ_pairs != 0 and summ_pairs < mn:
#         mn = summ_pairs
#         res_s = s
# counting = {i: res_s.count(i) for i in res_s}.items()
# mxc = [i[1] for i in counting]
# res = sorted([i for i in counting if i[1] == max(mxc)], key=lambda x: x[0], reverse=True)[0][0]
# print('{}{}'.format(res, ''.join(lines).count(res)))


'''
23. (Д. Муфаззалов) На отрезке [20222022; 50222022] найдите пять наибольших натуральных чисел с
суммой цифр, кратной числу 22, факториал каждого из которых имеет количество простых делителей,
кратное числу 2022. Выведите найденные числа в порядке убывания, справа от каждого числа –
количество простых делителей его факториала.
'''
# # TODO: Ответ –
# #  50207161 3012780
# #  50171116 3010758
...
# Поиск простых делителей с помощью "Решето Эратосфена"
top = 50222022
primes = [True] * top
i = 2
...
# Поиск кол-ва делителей факториала числа
# (кол-во делителей факториала числа n равно кол-ву делителей чисел до числа n и включая его)
counts = [0] * top
...
# Поиск ответа
c = 0
for i in range(50222022, 20222021, -1):
    s = sum(map(int, str(i)))
    if s % 22 != 0:
        continue
    if counts[i] % 2022 == 0:
...
Входные данные. В первой строке входного файла 26-6.txt находятся два числа: S – размер
...
'''
25. (Е. Драчева) Дана последовательность из N натуральных чисел. Рассматриваются всевозможные
пары различных элементов последовательности, между которыми есть хотя бы одно число, при этом
сумма пары кратна трём, а сумма чисел между ними чётна. Найдите количество таких пар.
Входные данные. Даны два входных файла (файл A и файл B), каждый из которых содержит в первой
строке количество чисел N (2 ≤ N ≤ 500000). Каждая из следующих N строк содержит натуральное
число, не превышающее 10000.
Пример входного файла:
7
1
3
...
#             if not (prefix[j] - prefix[i + 1]) & 1 and not (data[i] + data[j]) % 3:
#                 c += 1
#     return c
#
# with open('data/27-76t.txt') as f:
#     n = int(f.readline())
#     data = list(map(int, f.readlines()))
# print(func(n, data))
#
# with open('data/27-76a.txt') as f:
#     n = int(f.readline())
#     data = list(map(int, f.readlines()))
# print(func(n, data))
#
# with open('data/27-76b.txt') as f:
#     n = int(f.readline())
#     data = list(map(int, f.readlines()))
# print(func(n, data))
