...
#
#
# with open('data/22-5-P1.csv') as f:
#     rdr1 = [i for i in reader(f, delimiter=';', quotechar='"')]
# data1 = {}
# for c in rdr1[1:]:
#     data1[int(c[0])] = {'t': int(c[1]),
...
#     p1 = max(p1, f)
#
# with open('data/22-5-P2.csv') as f:
#     rdr2 = [i for i in reader(f, delimiter=';', quotechar='"')]
# data2 = {}
# for c in rdr2[1:]:
#     data2[int(c[0])] = {'t': int(c[1]),
#                         'r': [int(x) for x in c[2].split(';')]}
# p2 = 0
# for i in data2:
#     f = calc(data2, i)
...
"""
22. Текстовый файл 24-197.txt содержит строку из заглавных латинских букв X, Y и Z, всего не более
чем из 10**6 символов. Определите максимальное количество идущих подряд троек символов X*Y или
Z*Y, где * обозначает один любой символ.
"""
# # TODO: Ответ – 20
# with open('data/24-197.txt') as f:
#     s = f.readline()
# mx = 0
...
"""
23. (А. Богданов) Найдите наименьшее натуральное число, которое имеет ровно 1000 делителей. В
ответе запишите сначала само число и затем его наибольший простой делитель. Подсказка:
используйте основную теорему арифметики.
"""
# # TODO: Ответ – 810810000 13
# #  Решение:
# #  По основной теореме арифметики (a=p^x1 * p^x2 ... p^xn)
# #  и следующей из неё о количестве натуральных делителей ((x1 + 1) * (x2 + 1) * ... (xn + 1)) сформируем правило:
# #  1000 = 5 * 5 * 5 * 2 * 2 * 2  => x1 = x2 = x3 = 4; x4 = x5 = x6 = 1
# #  Следовательно наименьшее натуральное число равняется 2^4 * 3^4 * 5^4 * 7^1 * 11^1 * 13^1 (самое оптимальное)
# print(2**4 * 3**4 * 5**4 * 7 * 11 * 13, 13)


"""
24. (Досрочный ЕГЭ-2022) В лесополосе осуществляется посадка деревьев: саженцы высаживают
рядами на одинаковом расстоянии. Спустя некоторое время с помощью аэросъемки выясняют, какие
саженцы прижились. Необходимо определить ряд с максимальным номером, в котором есть подряд
ровно K неприжившихся саженцев при условии, что справа и слева от них саженцы прижились.
Входные данные представлены в файле 26-79.txt следующим образом. . В первой строке записаны
два числа: N – количество занятых мест (натуральное число, не превышающее 10 000) и K – длина
цепочки неприжившихся саженцев, которую нужно найти. Каждая из следующих N строк содержит
сведения об одном прижившемся саженце – два натуральных числа, не превышающих 100 000: номер
ряда и номер саженца в ряду.
В ответе запишите сначала наибольший номер ряда, затем наименьший номер неприжившегося
саженца.
...
"""
25. Набор данных состоит из нечётного количества пар натуральных чисел. Необходимо выбрать из
каждой пары ровно одно число так, чтобы сумма выбранных чисел была минимальной при условии,
что чётность этой суммы НЕ совпадает с чётностью большинства выбранных чисел. Определите
минимальную сумму, которую можно получить при таком условии. Гарантируется, что
...
строке количество чисел N (1 ≤ N ≤ 100000). Каждая из следующих N строк содержит два
...
7 2
9 14
Для указанных данных надо выбрать числа 7, 6, 6, 2 и 14. Большинство из них чётны, их сумма 35
нечётна.
...
def solution(data):
    parity = [[], []]
    for i in data:
        parity[sum(i) % 2].append(sum(i))
    if len(parity[0]) > len(parity[1]):
        return min(parity[1])
    else:
        return min(parity[0])


with open('data/27-51a.txt') as f:
    n = int(f.readline())
    data = list(map(lambda x: tuple(map(int, x.split())), f.readlines()))
print(solution(data), end=' ')
with open('data/27-51b.txt') as f:
    n = int(f.readline())
    data = list(map(lambda x: tuple(map(int, x.split())), f.readlines()))
print(solution(data))
